...
A simple C benchmark is included to measure memory read/write bandwidth:

```
gcc -O3 -march=native -o memory_bandwidth code/memory_bandwidth.c
./memory_bandwidth [buffer_size_mb] [iterations]
```

* `buffer_size_mb` (optional) controls the buffer size in megabytes (default: 256 MB).
* `iterations` (optional) runs the benchmark multiple times and reports the best bandwidth (default: 5).
